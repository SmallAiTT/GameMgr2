...
function getCnd(cnd, args, cb){
    ...
    if(arguments.length == 1){
        return {where : "", args : [], cb : arguments[0]}
...
function insert(poClass, args, cb){
    if(arguments.length < 3) throw "args error for insert";
...
function update(poClass, values, cnd, args, cb){
    if(arguments.length < 3) throw "args error for update";
    var tableName = typeof poClass == "string" ? poClass : poClass.tableName;
...
function select(poClass, cnd, args, cb){
    if(arguments.length < 2) throw "args error for select";
    var tableName = typeof poClass == "string" ? poClass : poClass.tableName;
...
function del(poClass, cnd, args, cb){
    if(arguments.length < 2) throw "args error for del";
    var tableName = typeof poClass == "string" ? poClass : poClass.tableName;
    var cnd = getCnd.apply(this, Array.prototype.slice.call(arguments, 1));
...
function list(poClass, cnd, args, cb){
    if(arguments.length < 2) throw "args error for list";
    var tableName = typeof poClass == "string" ? poClass : poClass.tableName;
    var cnd = getCnd.apply(this, Array.prototype.slice.call(arguments, 1));
    var strSql = "select * from	" + tableName + cnd.where;
 ...
 * @param cb
 */
function listPage(poClass, page, num, cnd, args, cb){
    if(arguments.length < 4) throw "args error for listPage";
    var tableName = typeof poClass == "string" ? poClass : poClass.tableName;
    var cnd = getCnd.apply(this, Array.prototype.slice.call(arguments, 3));
    var tempStr = tableName + cnd.where;
    var strSql = "select count(*) count from	" + tempStr;
    var self = this;
    self.query(strSql, cnd.args, function(err, result){
        var total = result[0].count;
        strSql = "select * from " + tempStr + " limit " + (num*(page - 1)) + "," + num;
        self.query(strSql, cnd.args, function(err, result){
            err ? cnd.cb(err) : cnd.cb(null, new PagePo(total, result));
        });
    });
}
...
function initDbOper(target){
    ...
    target.listPage = target.listPage ? target.listPage : listPage;
}
